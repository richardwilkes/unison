...
func PrimaryDisplay() *Display {
	...
	return convertDisplay(&d)
}

func convertDisplay(d *C.plafDisplay) *Display {
	return &Display{
		Frame:   geom.NewRect(float32(d.FrameX), float32(d.FrameY), float32(d.FrameWidth), float32(d.FrameHeight)),
		Usable:  geom.NewRect(float32(d.UsableX), float32(d.UsableY), float32(d.UsableWidth), float32(d.UsableHeight)),
		Scale:   geom.Point{X: float32(d.ScaleX), Y: float32(d.ScaleY)},
		PPI:     float32(d.PPI),
