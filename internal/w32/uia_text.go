...
// about itself, and where on screen it is. Like uia_map.go and uia_patterns.go it is a pure function of the snapshot —
// nothing here touches the OS, allocates COM memory or depends on which thread it runs on — so the file carries no
// build constraint and its tests run on any platform. uia_text_windows.go and uia_textrange_windows.go do nothing but
...
// — and UIATextClip is what deals with that.

// uiaTextUnitCount is how many text units UI Automation defines, which is what indexes the divisions of a document by
// the unit they divide it into.
const uiaTextUnitCount = int(TextUnit_Document) + 1

// uiaTextDocument is the view of one Document node's composed stream that the Text pattern answers from: the stream's
...
// remembered for the snapshot it was built from; see uiaMemoizedTextDocument. Everything it needs is worked out up
...
// at all (see uiaRolePatterns), so nothing here is ever reached for it.
type uiaTextDocument struct {
	tree       *accessibility.Tree
	node       *accessibility.Node
	info       *accessibility.TextInfo
	runes      []rune
	spans      []uiaSpan
	boundaries [uiaTextUnitCount][]int // the Character unit's entry is left nil; see uiaBoundaries
	visible    geom.Rect
	length     int
	focused    bool
}

// uiaSpan is one element's stretch of a document's stream, along with where it sits among the others. It is the
...
// uiaNewTextDocument builds the view of a Document node's stream, or nil when the node carries none.
func uiaNewTextDocument(t *accessibility.Tree, n *accessibility.Node) *uiaTextDocument {
	if t == nil || n == nil || n.Document == nil {
		return nil
	}
	d := &uiaTextDocument{tree: t, node: n, info: &n.Document.Text}
	d.runes = []rune(d.info.Text)
	d.length = len(d.runes)
	d.visible = uiaVisibleBounds(t, n)
	d.focused = UIAHasKeyboardFocus(t, n)
	d.spans = uiaReportedSpans(t, d.info.Spans)
	for unit := range uiaTextUnitCount {
...
// uiaReportedSpans turns the snapshot's spans into the ones a client can be told about, each knowing which span
...
func uiaReportedSpans(t *accessibility.Tree, spans []accessibility.TextSpan) []uiaSpan {
	if len(spans) == 0 {
		return nil
	}
	reported := make([]uiaSpan, 0, len(spans))
	for _, span := range spans {
		if n := t.Node(span.Node); n == nil || n.Ignored {
			continue
		}
		parent := -1
		for i := range reported {
			if reported[i].start <= span.Start && reported[i].end >= span.End {
				parent = i
			}
		}
		reported = append(reported, uiaSpan{node: span.Node, start: span.Start, end: span.End, parent: parent})
	}
	return reported
}

// uiaMemoizedTextDocument returns the view of one Document node's stream, working it out and remembering it whenever
// uiaSnapshotMemo does not already hold one for this snapshot. It answers nil for a node that is not a Document or
// carries no stream.
//
// A snapshot the memo will not serve is answered from itself, outside the lock and remembering nothing, exactly as the
// memo's other answers are; see uiaMemoSwitchTo for which snapshots those are and why.
//
...
func uiaMemoizedTextDocument(t *accessibility.Tree, id accessibility.NodeID) *uiaTextDocument {
	if t == nil {
		return nil
	}
	node := t.Node(id)
	...
	serving := uiaMemoSwitchTo(t)
	var held *uiaTextDocument
	if serving {
		held = uiaSnapshotMemo.documents[id]
	}
	uiaSnapshotMemo.lock.Unlock()
	if !serving {
		return uiaNewTextDocument(t, node)
	}
	if held != nil {
		return held
	}
	doc := uiaNewTextDocument(t, node)
	uiaSnapshotMemo.lock.Lock()
	...
	if held = uiaSnapshotMemo.documents[id]; held != nil {
		return held
	}
	if uiaSnapshotMemo.documents == nil {
		uiaSnapshotMemo.documents = make(map[accessibility.NodeID]*uiaTextDocument)
	}
	uiaSnapshotMemo.documents[id] = doc
	return doc
}

// uiaTextContainerFor returns the id of the document whose stream a node sits inside, or zero when it sits in none. It
...
func uiaTextContainerFor(t *accessibility.Tree, n *accessibility.Node) accessibility.NodeID {
	if t == nil || n == nil || n.Document != nil {
		return 0
	}
	id := n.ID
	for depth := 0; depth < uiaMaxTreeDepth; depth++ {
		...
		if parent.Document != nil {
			if _, _, ok := uiaTextSpanFor(t, parent, n.ID); ok {
				return parentID
			}
			return 0
		}
...
// uiaTextSpanFor returns the stretch of a document's stream that a node occupies, and whether it occupies one. A node
...
//
// It works from the spans alone rather than from a uiaTextDocument, so that granting the TextChild pattern — which
...
// them; see uiaMemoizedTextSpans.
func uiaTextSpanFor(t *accessibility.Tree, doc *accessibility.Node, id accessibility.NodeID) (start, end int, ok bool) {
	if t == nil || doc == nil || doc.Document == nil || id == 0 || id == doc.ID {
		return 0, 0, false
	}
	index := uiaMemoizedTextSpans(t)
	for depth := 0; depth < uiaMaxTreeDepth; depth++ {
		if spanStart, spanEnd, found := uiaSpanOf(index, doc, id); found {
...
// uiaSpanOf returns the stretch of one document's stream that a node's own span records, and whether it has one. It
...
func uiaSpanOf(index map[accessibility.NodeID]uiaDocumentSpan, doc *accessibility.Node, id accessibility.NodeID,
) (start, end int, ok bool) {
	if index != nil {
		span, held := index[id]
		if !held || span.document != doc.ID {
			return 0, 0, false
		}
...
// uiaDocumentSpan is which document's stream an element occupies a stretch of, and which stretch of it. It is what a
// snapshot's spans are indexed by element into; see uiaMemoizedTextSpans.
type uiaDocumentSpan struct {
	document accessibility.NodeID
	start    int
	end      int
}

// uiaMemoizedTextSpans returns the index of which stretch of which document's stream each element of a snapshot
// occupies, working it out and remembering it whenever uiaSnapshotMemo does not already hold one for this snapshot.
//
// It answers nil for a snapshot in which no document carries a span, and for a snapshot the memo will not serve — see
// uiaMemoSwitchTo for which those are and why — which has the caller answer from the document's own span list instead,
// exactly as it did before anything was remembered.
//
// The index is what keeps granting the TextChild pattern cheap. UIAProvidedPatterns asks where an element sits for
// every element a client so much as looks at — on every QueryInterface, every GetPatternProvider, every
// UIAReportsProperty check, and twice per node in patternAvailability — and a scan of a document's span list for each
...
func uiaMemoizedTextSpans(t *accessibility.Tree) map[accessibility.NodeID]uiaDocumentSpan {
	uiaSnapshotMemo.lock.Lock()
	defer uiaSnapshotMemo.lock.Unlock()
	if !uiaMemoSwitchTo(t) {
		return nil
	}
	if !uiaSnapshotMemo.textSpansDone {
		uiaSnapshotMemo.textSpans = uiaDocumentSpans(t)
		uiaSnapshotMemo.textSpansDone = true
	}
	return uiaSnapshotMemo.textSpans
}

// uiaDocumentSpans indexes every stretch of every document's stream in a snapshot by the element that occupies it, or
...
func uiaDocumentSpans(t *accessibility.Tree) map[accessibility.NodeID]uiaDocumentSpan {
	var index map[accessibility.NodeID]uiaDocumentSpan
	for _, n := range t.Nodes {
		if n.Document == nil {
			continue
		}
		for _, span := range n.Document.Text.Spans {
			if index == nil {
				index = make(map[accessibility.NodeID]uiaDocumentSpan)
			}
			if _, held := index[span.Node]; !held {
				index[span.Node] = uiaDocumentSpan{document: n.ID, start: span.Start, end: span.End}
			}
		}
	}
	return index
...
// uiaVisibleBounds returns the part of a node's bounds that is really on screen: its own bounds intersected with those
...
func uiaVisibleBounds(t *accessibility.Tree, n *accessibility.Node) geom.Rect {
...
// grown shorter since.
func (d *uiaTextDocument) ClampOffset(offset int) int {
...
func (d *uiaTextDocument) ClampRange(start, end int) (clampedStart, clampedEnd int) {
...
func (d *uiaTextDocument) Text(start, end int) string {
...
// selected.
func (d *uiaTextDocument) Selection() (start, end int) {
...
func (d *uiaTextDocument) Caret() int {
...
func (d *uiaTextDocument) Focused() bool {
	return d.focused
}
...
func (d *uiaTextDocument) SupportedSelection() SupportedTextSelection {
...
// a provider asked for a unit it does not support: report the next larger one it does. uiaTextUnitValid is what refuses
// a value that is not a unit at all.
func (d *uiaTextDocument) Boundaries(unit TextUnit) []int {
...
func (d *uiaTextDocument) boundariesFor(unit TextUnit) uiaBoundaries {
	if !uiaTextUnitValid(unit) {
		unit = TextUnit_Document
	}
	if unit == TextUnit_Character {
		return uiaBoundaries{length: d.length, runes: true}
	}
	return uiaBoundaries{offsets: d.boundaries[unit], length: d.length}
}

// uiaBoundaries is where one unit divides a stream, in the three terms every operation in this file is written in: how
...
func (b uiaBoundaries) At(i int) int {
	if b.runes {
		return i
	}
	return b.offsets[i]
...
func (b uiaBoundaries) Search(offset int) (i int, found bool) {
	if b.runes {
		switch {
		case offset < 0:
			return 0, false
...
func (b uiaBoundaries) Offsets() []int {
	if !b.runes {
		return b.offsets
	}
...
// uiaTextUnitValid reports whether a value a client passed is one of the text units UI Automation defines. Anything
// else is a programming error on the client's side, which the provider answers with E_INVALIDARG rather than guessing
// at.
func uiaTextUnitValid(unit TextUnit) bool {
	return unit >= TextUnit_Character && unit <= TextUnit_Document
}

// uiaTextEndpointValid reports whether a value a client passed names one end of a range. It is the companion of
// uiaTextUnitValid, and is refused in the same places and for the same reason: a method that took anything outside the
// enumeration for the start would move or compare the wrong end of the range and report success, which a client has no
// way to notice.
func uiaTextEndpointValid(endpoint TextPatternRangeEndpoint) bool {
...
// are every offset from zero to the length of the stream, which uiaBoundaries answers without a slice — and UI
...
func (d *uiaTextDocument) computeBoundaries(unit TextUnit) []int {
	switch unit {
	case TextUnit_Word:
		return uiaSortedBoundaries(d.length, textunit.WordStarts(d.runes))
	case TextUnit_Line:
		...
		return uiaSortedBoundaries(d.length, offsets)
	case TextUnit_Paragraph:
		offsets := textunit.ParagraphStarts(d.runes)
		for i := range d.spans {
			if uiaSpanIsParagraph(d.tree.Node(d.spans[i].node)) {
	...
	case TextUnit_Format:
		...
		return uiaSortedBoundaries(d.length, offsets)
	default:
		return uiaSortedBoundaries(d.length, nil)
	}
}

// uiaSpanIsParagraph reports whether the element a span stands for begins a paragraph of the stream. These are the
...
func uiaSpanIsParagraph(n *accessibility.Node) bool {
	if n == nil {
		return false
	}
	switch n.Role {
...
// uiaSortedBoundaries returns the offsets a unit divides a stream of the given length at: those of the offsets handed
...
func uiaSortedBoundaries(length int, offsets []int) []int {
...
func (d *uiaTextDocument) UnitStart(unit TextUnit, offset int) int {
...
func (d *uiaTextDocument) UnitEnd(unit TextUnit, offset int) int {
	bounds := d.boundariesFor(unit)
	i, found := bounds.Search(d.ClampOffset(offset))
	if found {
		i++
...
func (d *uiaTextDocument) IsUnitBoundary(unit TextUnit, offset int) bool {
...
func (d *uiaTextDocument) Expand(unit TextUnit, start, end int) (expandedStart, expandedEnd int) {
	start, end = d.ClampRange(start, end)
	if d.length == 0 {
		return 0, 0
	}
...
func (d *uiaTextDocument) Move(unit TextUnit, count, start, end int) (movedStart, movedEnd, moved int) {
	start, end = d.ClampRange(start, end)
	if start == end {
...
func (d *uiaTextDocument) step(unit TextUnit, at, count, limit int) (offset, moved int) {
	bounds := d.boundariesFor(unit)
...
// that runs backwards.
func (d *uiaTextDocument) MoveEndpoint(unit TextUnit, endpoint TextPatternRangeEndpoint, count, start, end int,
...
// and Move.
func (d *uiaTextDocument) stepEndpoint(unit TextUnit, at, count int) (offset, moved int) {
	if count == 0 {
		return at, 0
	}
	bounds := d.boundariesFor(unit)
...
func (d *uiaTextDocument) OffsetAt(pt geom.Point) int {
	if len(d.info.Lines) == 0 {
		return 0
	}
	...
	return d.ClampOffset(uiaOffsetOnLine(line, pt.X-line.Bounds.X, d.length))
}

// uiaOffsetOnLine returns the offset of the character at a horizontal position on a line, measured from the line's own
...
func uiaOffsetOnLine(line *accessibility.Line, x float32, count int) int {
...
func (d *uiaTextDocument) VisibleRange() (start, end int, ok bool) {
	if d.visible.Empty() {
		return 0, 0, false
	}
	if len(d.info.Lines) == 0 {
...
func (d *uiaTextDocument) Rectangles(start, end int) []geom.Rect {
	start, end = d.ClampRange(start, end)
...
func (d *uiaTextDocument) lineBounds(line *accessibility.Line) geom.Rect {
	bounds := line.Bounds
...
func (d *uiaTextDocument) lineRect(line *accessibility.Line, from, to int) geom.Rect {
...
func (d *uiaTextDocument) EnclosingSpan(start, end int) accessibility.NodeID {
...
func (d *uiaTextDocument) enclosingSpanIndex(start, end int) int {
	start, end = d.ClampRange(start, end)
	if start == end {
...
func (d *uiaTextDocument) Children(start, end int) []accessibility.NodeID {
	start, end = d.ClampRange(start, end)
	if start == end {
		return nil
	}
	...
	for i := range d.spans {
		span := &d.spans[i]
		if span.parent != parent || span.end <= start || span.start >= end {
			continue
		}
		ids = append(ids, span.node)
	}
	return ids
}

// SpanFor returns the stretch of the stream an element occupies, and whether it occupies one. See uiaTextSpanFor, which
...
func (d *uiaTextDocument) SpanFor(id accessibility.NodeID) (start, end int, ok bool) {
	return uiaTextSpanFor(d.tree, d.node, id)
}

// uiaAttributeKind says what sort of value an answer about a text attribute carries, which is what decides how the
// Windows layer stores it into the VARIANT a client reads it from.
type uiaAttributeKind uint8

// Possible uiaAttributeKind values. The first two are the reserved values UI Automation defines for the two things a
// provider may have to say instead of a value: that it will never answer this attribute, and that the range covers text
// the attribute differs over. See uiaTextRangeGetAttributeValue.
const (
	uiaAttributeUnsupported uiaAttributeKind = iota
	uiaAttributeMixed
	uiaAttributeEmpty
	uiaAttributeBoolean
	uiaAttributeInteger
...
// uiaAttribute is what a range has to say about one text attribute. Which fields carry information depends on Kind; the
...
func (d *uiaTextDocument) Attribute(start, end int, attribute TextAttributeID) uiaAttribute {
	start, end = d.ClampRange(start, end)
	var answer uiaAttribute
	for i, offset := range d.formatOffsets(start, end) {
		value := d.attributeAt(offset, attribute)
		if i == 0 {
			answer = value
			continue
		}
		if value != answer {
			return uiaAttribute{Kind: uiaAttributeMixed}
		}
	}
	return answer
}
...
func (d *uiaTextDocument) FindAttribute(start, end int, attribute TextAttributeID, want uiaAttribute, backward bool,
) (foundStart, foundEnd int, ok bool) {
	start, end = d.ClampRange(start, end)
	if start >= end || !uiaAttributeSearchable(want.Kind) {
		return 0, 0, false
	}
	...
	for i, offset := range offsets {
		...
		switch {
		case d.attributeAt(offset, attribute) != want:
			if inStretch {
...
// uiaAttributeSearchable reports whether a value is one a stretch of text can be looked for by. The reserved mixed and
...
// anything.
func uiaAttributeSearchable(kind uiaAttributeKind) bool {
	switch kind {
	case uiaAttributeBoolean, uiaAttributeInteger, uiaAttributeNumber, uiaAttributeString:
		return true
	default:
		return false
	}
...
func (d *uiaTextDocument) formatOffsets(start, end int) []int {
	if d.length == 0 {
		return []int{0}
	}
	if start >= end {
...
// asks it, and uia_constants.go for which attributes are answered and which are deliberately refused.
func (d *uiaTextDocument) attributeAt(offset int, attribute TextAttributeID) uiaAttribute {
	run := d.runAt(offset)
	switch attribute {
	case UIA_FontNameAttributeId:
		...
		return uiaAttribute{Kind: uiaAttributeString, Text: run.Family}
	case UIA_FontSizeAttributeId:
		if run == nil || run.Size <= 0 {
			return uiaAttribute{Kind: uiaAttributeUnsupported}
		}
		return uiaAttribute{Kind: uiaAttributeNumber, Number: float64(run.Size)}
	case UIA_FontWeightAttributeId:
		if run == nil || run.Weight <= 0 {
			return uiaAttribute{Kind: uiaAttributeUnsupported}
		}
		return uiaAttribute{Kind: uiaAttributeInteger, Int: int32(run.Weight)}
	case UIA_IsItalicAttributeId:
		return uiaAttribute{Kind: uiaAttributeBoolean, Bool: run != nil && run.Italic}
	case UIA_UnderlineStyleAttributeId:
		return uiaAttribute{Kind: uiaAttributeInteger, Int: int32(uiaLineStyle(run != nil && run.Underline))}
	case UIA_StrikethroughStyleAttributeId:
		return uiaAttribute{Kind: uiaAttributeInteger, Int: int32(uiaLineStyle(run != nil && run.Strikethrough))}
	case UIA_IsReadOnlyAttributeId:
		...
		return uiaAttribute{Kind: uiaAttributeBoolean, Bool: true}
	case UIA_IsHiddenAttributeId:
		...
		return uiaAttribute{Kind: uiaAttributeBoolean, Bool: false}
	case UIA_IsActiveAttributeId:
		return uiaAttribute{Kind: uiaAttributeBoolean, Bool: d.focused}
	case UIA_StyleIdAttributeId:
		style, _ := d.styleAt(offset)
		return uiaAttribute{Kind: uiaAttributeInteger, Int: int32(style)}
	case UIA_StyleNameAttributeId:
		if _, name := d.styleAt(offset); name != "" {
			return uiaAttribute{Kind: uiaAttributeString, Text: name}
		}
		return uiaAttribute{Kind: uiaAttributeUnsupported}
	case UIA_LinkAttributeId:
		if start, end, ok := d.linkAt(offset); ok {
			return uiaAttribute{Kind: uiaAttributeRange, Start: start, End: end}
		}
		...
		return uiaAttribute{Kind: uiaAttributeEmpty}
	default:
		return uiaAttribute{Kind: uiaAttributeUnsupported}
	}
}

// uiaLineStyle turns the presence of an underline or a strikethrough into the style UI Automation reports it as. A
// snapshot records that a run is decorated and not how the line is drawn, so the answer is the plain single line.
func uiaLineStyle(decorated bool) TextDecorationLineStyle {
	if decorated {
...
func (d *uiaTextDocument) runAt(offset int) *accessibility.TextRun {
...
func (d *uiaTextDocument) styleAt(offset int) (style StyleID, name string) {
	for i := len(d.spans) - 1; i >= 0; i-- {
		span := &d.spans[i]
		if span.start > offset || span.end <= offset {
			continue
		}
		n := d.tree.Node(span.node)
		if n == nil {
			continue
		}
		switch n.Role {
		case role.Heading:
			return uiaHeadingStyle(n.Level), ""
...
// uiaHeadingStyle returns the style identifier for a heading of the given level. UI Automation defines nine, so a
...
func uiaHeadingStyle(level int) StyleID {
	switch {
	case level < 1:
...
func (d *uiaTextDocument) linkAt(offset int) (start, end int, ok bool) {
	for i := len(d.spans) - 1; i >= 0; i-- {
		span := &d.spans[i]
		if span.start > offset || span.end <= offset {
			continue
		}
		if n := d.tree.Node(span.node); n != nil && n.Role == role.Link {
			return span.start, span.end, true
		}
	}
	return 0, 0, false
...
// direction.
func (d *uiaTextDocument) FindText(start, end int, text string, backward, ignoreCase bool) (
	foundStart, foundEnd int, ok bool,
) {
	start, end = d.ClampRange(start, end)
	needle := []rune(text)
	...
	if ignoreCase {
		haystack = uiaFoldRunes(haystack)
		needle = uiaFoldRunes(needle)
	}
	i := uiaIndexRunes(haystack, needle, backward)
	if i < 0 {
		return 0, 0, false
	}
...
// uiaFoldRunes returns the runes with each one lowercased on its own, which is what keeps the answer the same length as
...
func uiaFoldRunes(runes []rune) []rune {
...
// uiaIndexRunes returns the index of the first occurrence of needle within haystack, or of the last when backward is
// true, or -1 when there is none.
func uiaIndexRunes(haystack, needle []rune, backward bool) int {
...
// UIATextClip shortens a string to the maximum length ITextRangeProvider::GetText was given, which is a count of UTF-16
...
func UIATextClip(s string, maxLength int) string {
	if maxLength < 0 {
		return s
	}
	if maxLength == 0 {
