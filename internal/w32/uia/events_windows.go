...
//     here are.
//   - They cost something even when nobody is listening, so every one of them sits behind ClientsAreListening. A
...
//     Automation Core, which clears it; one passed into a Raise* function is copied by the call and stays ours, so
//     every one here is paired with a deferred Clear or Free.
//
// Which calls one publish makes is decided by DecideRaises, which is a pure function of the two snapshots and the
...
var (
	clientsAreListening                 = ClientsAreListening
	raiseAutomationEvent                = RaiseAutomationEvent
	raiseAutomationPropertyChangedEvent = RaiseAutomationPropertyChangedEvent
	raiseStructureChangedEvent          = RaiseStructureChangedEvent
	raiseNotificationEvent              = RaiseNotificationEvent
	disconnectProvider                  = DisconnectProvider
	returnRawElementProvider            = ReturnRawElementProvider
	hostProviderFromHwnd                = HostProviderFromHwnd
)

// announcementActivity names the activity every announcement this toolkit makes belongs to. A client that groups or
...
const announcementActivity = "unison"
...
// stands behind that.
func (w *Window) raiseEvents(old, cur *accessibility.Tree, events []accessibility.Event) {
	if len(events) == 0 && old != nil {
		return
	}
	if !clientsAreListening() {
		return
	}
	for _, raise := range DecideRaises(old, cur, events) {
		w.raiseOne(old, cur, raise)
	}
}

// raiseOne makes one of the calls DecideRaises asked for. The provider it names is created here if nothing has
...
func (w *Window) raiseOne(old, cur *accessibility.Tree, raise Raise) {
	if raise.Kind == RaiseDisconnect {
	...
	switch raise.Kind {
	case RaiseEvent:
		raiseAutomationEvent(p.Unknown(), raise.Event)
	case RaiseProperty:
		p.raisePropertyChanged(old, cur, raise.Property)
	case RaiseStructure:
		raiseStructureChangedEvent(p.Unknown(), raise.Change, structureRuntimeID(raise))
	default:
	}
}
...
func (w *Window) Announce(text string) {
	if text == "" || !clientsAreListening() {
		return
	}
...
// is reached from a publish, where DecideRaises has already decided what to say; this is for the one pattern method
// that has to report its own outcome, IInvokeProvider::Invoke, whose event no change to the snapshot need follow.
func (p *Provider) raiseEvent(eventID EventID) {
	if !clientsAreListening() {
		return
	}
	raiseAutomationEvent(p.Unknown(), eventID)
}

// structureRuntimeID returns the runtime identifier a structure change carries: the added or removed child's, since
// a removal is reported on a parent that has to say which child left. An invalidation names no child and carries none.
func structureRuntimeID(raise Raise) []int32 {
	if raise.Child == 0 {
		return nil
	}
	return RuntimeID(raise.Child)
}
...
func (p *Provider) raisePropertyChanged(old, cur *accessibility.Tree, propertyID PropertyID) {
	...
	raiseAutomationPropertyChangedEvent(p.Unknown(), propertyID, oldValue, newValue)
}
...
//
// ReportsProperty is what keeps that promise for the pattern properties, which have no GetPropertyValue answer to
...
// must never do. DecideRaises no longer raises a pattern's property on an element that has lost the pattern — the
...
func (p *Provider) raisedPropertyValue(tree *accessibility.Tree, propertyID PropertyID, value *VARIANT) {
	node := tree.Node(p.node)
	if node == nil || !ReportsProperty(tree, node, propertyID) {
		return
	}
	if pattern := AvailabilityPattern(propertyID); pattern != 0 {
		value.SetBool(ProvidesPattern(tree, node, pattern))
		return
	}
	switch propertyID {
	...
	case RangeValueValuePropertyId:
		value.SetR8(node.Number)
	case ValueIsReadOnlyPropertyId:
		value.SetBool(IsValueReadOnly(node))
	case RangeValueIsReadOnlyPropertyId:
		value.SetBool(IsRangeValueReadOnly(node))
	case ToggleToggleStatePropertyId:
		value.SetI4(int32(ToggleStateOf(node)))
	case ExpandCollapseExpandCollapseStatePropertyId:
		value.SetI4(int32(ExpandCollapseStateOf(node)))
	case SelectionItemIsSelectedPropertyId:
		value.SetBool(IsSelected(node))
	...
	case WindowIsModalPropertyId:
		...
		// through: it asks IWindowProvider::get_IsModal instead. Reaching this means ReportsProperty has already
...
// property in: an array of four doubles, rather than the Rect that
...
func (p *Provider) setBoundingRectangle(value *VARIANT, node *accessibility.Node) {
...
func (p *Provider) raiseNotification(text string) {
	display := NewBSTR(text)
	defer display.Free()
	activity := NewBSTR(announcementActivity)
	defer activity.Free()
	raiseNotificationEvent(p.Unknown(), NotificationKind_Other, NotificationProcessing_All, display, activity)
}
