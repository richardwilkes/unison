...
//	ITextProvider::RangeFromPoint(this, Point point, out) — on arm64 only
//
// The third is there because a Point is two doubles and nothing else, which the ARM64 convention passes in
// floating-point registers rather than by reference; on amd64 the same structure arrives as an ordinary pointer and
// needs none of this. Its halves live in text_point_windows_arm64.go and text_point_windows_amd64.go, and its
...
//
// Fallback. If a thunk ever misbehaves on some target, set thunksEnabled to false. Each of the three slots then
...
// thunksEnabled says whether the vtable slots that arrive with a double in a floating-point register use the
// assembly thunks. See the fallback note above for what turning it off costs.
const thunksEnabled = true
...
var (
	fromPointCallback          uintptr
	rangeValueSetValueCallback uintptr
)

// fromPointSlot returns what belongs in the ElementProviderFromPoint vtable slot.
func fromPointSlot() uintptr {
	if thunksEnabled {
		return fromPointThunkAddr()
	}
	return windows.NewCallback(floatArgumentUnavailable)
}

// rangeValueSetValueSlot returns what belongs in the IRangeValueProvider::SetValue vtable slot.
func rangeValueSetValueSlot() uintptr {
	if thunksEnabled {
		return rangeValueSetValueThunkAddr()
	}
	return windows.NewCallback(floatArgumentUnavailable)
}

// floatArgumentUnavailable is what a slot that takes a double answers when the thunks are turned off. It declares no
...
func floatArgumentUnavailable() uint64 {
	return E_NOTSUPPORTED
}

// fromPointThunkAddr returns the address of the ElementProviderFromPoint thunk, for the vtable slot. Implemented in
// thunk_windows_amd64.s and thunk_windows_arm64.s.
func fromPointThunkAddr() uintptr

// rangeValueSetValueThunkAddr returns the address of the IRangeValueProvider::SetValue thunk, for the vtable slot.
// Implemented in thunk_windows_amd64.s and thunk_windows_arm64.s.
func rangeValueSetValueThunkAddr() uintptr

// fromPointShimAddr returns the address of the shim that calls the ElementProviderFromPoint thunk the way UI
...
// rangeValueSetValueShimAddr returns the address of the shim that calls the IRangeValueProvider::SetValue thunk the
// way UI Automation does, for the same reason fromPointShimAddr exists. Implemented in
// thunk_windows_amd64.s and thunk_windows_arm64.s.
