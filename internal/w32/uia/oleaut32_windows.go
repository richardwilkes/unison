...
var (
	...
	// The BSTR allocator is declared in variant_windows.go rather than here, so that the one place a BSTR can be
	// born sits beside the comment explaining who then owns it. A hygiene test in the parent package's w32_test.go
	// keeps it there.
