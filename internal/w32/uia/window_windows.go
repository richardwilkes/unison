...
//
// One Window serves one HWND, and is created the first time that window is asked for a UI Automation provider. It
...
// Config holds what the UI Automation adapter for a window needs from the window itself.
type Config struct {
...
// Window is the UI Automation adapter for one window: the fragment root, the providers for the nodes beneath it, and
// the snapshot they all answer from.
type Window struct {
	tree      atomic.Pointer[accessibility.Tree]
	geometry  atomic.Pointer[Geometry]
	action    func(request accessibility.ActionRequest)
	providers map[accessibility.NodeID]*Provider
	root      *Provider
...
// NewWindow creates the UI Automation adapter for one window, with the snapshot its providers will answer from and
...
// snapshot.
func NewWindow(cfg Config, tree *accessibility.Tree, geometry Geometry) *Window {
	w := &Window{
		action:    cfg.Action,
		providers: make(map[accessibility.NodeID]*Provider),
		hwnd:      cfg.HWND,
	}
...
func (w *Window) HWND() windows.HWND {
	return w.hwnd
}
...
func (w *Window) Tree() *accessibility.Tree {
	return w.tree.Load()
}
...
// diagnostics and tests only: whether to raise an event is decided by ClientsAreListening, which knows about clients
// this fragment was never told about.
func (w *Window) Listeners() int32 {
...
// ReturnRawElementProvider answers a WM_GETOBJECT with, or nil once the window has been destroyed. The pointer is
// unowned: ReturnRawElementProvider takes a reference of its own, and until it does, the provider map's reference
...
func (w *Window) RootUnknown() unsafe.Pointer {
	root := w.Root()
	if root == nil {
		return nil
	}
...
func (w *Window) Provider(id accessibility.NodeID) *Provider {
	if id == 0 {
		return nil
	}
	w.lock.Lock()
	...
	p := newProvider(w, id)
	w.providers[id] = p
	// One reference for the map, which newProvider left behind, and one for the caller.
	p.addRef()
	return p
}
...
// reported on the parent while the child is still there to be disconnected. See raiseEvents in events_windows.go.
func (w *Window) Publish(tree *accessibility.Tree, events []accessibility.Event) {
	if tree == nil {
		return
	}
...
// E_ELEMENTNOTAVAILABLE, telling UI Automation to drop the references it holds to it, and giving up the reference
...
func (w *Window) retireRemovedProviders(tree *accessibility.Tree) {
	...
	w.lock.Unlock()
	// Deliberately outside the lock: DisconnectProvider calls into UI Automation, which may call back into a
...
// describe the window one last time has to find the window's content still there. ReturnRawElementProvider with a
...
//
// Providers a client still holds stay alive but answer E_ELEMENTNOTAVAILABLE, and the adapter itself answers
...
func (w *Window) Destroy() {
	...
	defer forgetSnapshotMemo()
	root := w.Root()
	if root != nil {
		if clientsAreListening() {
			raiseAutomationEvent(root.Unknown(), Window_WindowClosedEventId)
		}
		if w.hwnd != 0 {
			...
			returnRawElementProvider(w.hwnd, 0, 0, nil)
		}
	}
	w.lock.Lock()
	retired := make([]*Provider, 0, len(w.providers))
	for id, p := range w.providers {
		if id == w.rootNode {
...
func (w *Window) adviseEvents(delta int32) {
...
func (w *Window) dispatch(request accessibility.ActionRequest) bool {
	if w.action == nil {
		return false
	}
	w.action(request)
