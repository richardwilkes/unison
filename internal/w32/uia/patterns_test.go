...
// keeping the questions in patterns.go: the Windows-only tests then have only the COM left to check.
...
//
// The header is nowhere inside the table, which is the situation TableColumnHeaders exists for. Rows 0, 3 and 4 are
...
// TestSelectionItemRole verifies which role a selection container selects among, which is what decides where
// GetSelection looks.
func TestSelectionItemRole(t *testing.T) {
	c := check.New(t)
	c.Equal(role.ListItem, SelectionItemRole(role.List))
	c.Equal(role.Tab, SelectionItemRole(role.TabList))
	c.Equal(role.Row, SelectionItemRole(role.Table))
	c.Equal(role.Row, SelectionItemRole(role.Tree))
	for _, r := range []role.Enum{role.Window, role.Group, role.Row, role.ListItem, role.Cell, role.RadioButton} {
		c.Equal(role.None, SelectionItemRole(r), "role %s selects nothing", r.Key())
	}
}

// TestIsSelected verifies the state a selection item reports. A radio button has no selected state of its own:
// being the chosen one of its group is being checked.
func TestIsSelected(t *testing.T) {
	c := check.New(t)
	tree := patternTree()
	c.True(IsSelected(tree.Node(6)))
	c.False(IsSelected(tree.Node(7)))
	list := listTree()
	c.True(IsSelected(list.Node(3)))
	c.False(IsSelected(list.Node(6)))
	c.False(IsSelected(nil))
	...
	c.False(IsSelected(tree.Node(6)))
}

// TestSelection verifies the set of elements a container reports as selected, including that an ignored node between
// the container and an item is spliced away rather than hiding it.
func TestSelection(t *testing.T) {
	c := check.New(t)
	list := listTree()
	c.Equal([]accessibility.NodeID{3, 5}, Selection(list, 2))
	c.Equal([]accessibility.NodeID{8}, Selection(list, 7))
	c.Nil(Selection(list, 3), "a list item is not a selection container")
	c.Nil(Selection(list, 1), "a window is not a selection container")
	c.Nil(Selection(list, 999))
	c.Nil(Selection(nil, 2))
	...
	c.Nil(Selection(list, 2))

	table := tableTree()
	c.Equal([]accessibility.NodeID{7}, Selection(table, 6))
	table.Nodes[10].Selected = true
	c.Equal([]accessibility.NodeID{7, 10}, Selection(table, 6))
}

// TestSelectionNested verifies that a container reports its own selection and not a nested container's: a table
...
func TestSelectionNested(t *testing.T) {
	...
	c.Equal([]accessibility.NodeID{7}, Selection(tree, 6))
	c.Equal([]accessibility.NodeID{14}, Selection(tree, 13))
	...
	c.Equal([]accessibility.NodeID{7, 13}, Selection(nested, 6))
}

// malformedTreeDeadline is how long a question asked of a malformed snapshot is given before the test calls it hung.
...
const malformedTreeDeadline = 10 * time.Second

// TestSelectionMalformedTree verifies that the walk down a subtree both terminates and answers on a snapshot whose
...
func TestSelectionMalformedTree(t *testing.T) {
	c := check.New(t)
	...
	c.Equal([]accessibility.NodeID{bottom}, Selection(newTestTree(1, 0, nodes...), 1))
	...
	go func() { done <- Selection(cycle, 1) }()
	select {
	case ids := <-done:
		c.Equal([]accessibility.NodeID{4}, ids)
	case <-time.After(malformedTreeDeadline):
		t.Fatal("Selection followed the cyclic child links rather than stopping at the nodes it had seen")
	}
}

// TestTableColumnHeadersMalformedTree verifies that the search for a table's header survives a snapshot that lists
...
// there unfound.
func TestTableColumnHeadersMalformedTree(t *testing.T) {
	c := check.New(t)
	...
	c.Equal([]accessibility.NodeID{5, 6}, TableColumnHeaders(tree, 4))
}

// TestSelectionContainer verifies which element a selection item says it belongs to, including that a radio button
// says none: its group is a layout panel with no pattern of its own.
func TestSelectionContainer(t *testing.T) {
	c := check.New(t)
	list := listTree()
	c.Equal(accessibility.NodeID(2), SelectionContainer(list, 3))
	c.Equal(accessibility.NodeID(2), SelectionContainer(list, 5), "through an ignored group")
	c.Equal(accessibility.NodeID(7), SelectionContainer(list, 8))
	c.Equal(accessibility.NodeID(0), SelectionContainer(list, 1))
	c.Equal(accessibility.NodeID(0), SelectionContainer(list, 999))

	table := tableTree()
	c.Equal(accessibility.NodeID(6), SelectionContainer(table, 7))

	tree := patternTree()
	c.Equal(accessibility.NodeID(0), SelectionContainer(tree, 6), "a radio button reports no container")
	c.Equal(accessibility.NodeID(0), SelectionContainer(tree, 2))
}

// TestContainingGrid verifies which element a cell says it belongs to.
func TestContainingGrid(t *testing.T) {
	c := check.New(t)
	table := tableTree()
	c.Equal(accessibility.NodeID(6), ContainingGrid(table, 8))
	c.Equal(accessibility.NodeID(6), ContainingGrid(table, 12))
	c.Equal(accessibility.NodeID(6), ContainingGrid(table, 7), "a row is inside the grid too")
	c.Equal(accessibility.NodeID(0), ContainingGrid(table, 6), "a table is not inside itself")
	c.Equal(accessibility.NodeID(0), ContainingGrid(table, 4))
	c.Equal(accessibility.NodeID(0), ContainingGrid(listTree(), 3))
}

// TestGridItem verifies the cell lookup a client walking a table by row and column goes through. A cell the snapshot
// does not hold is reported as nothing, which is the normal answer for a table that publishes only its viewport.
func TestGridItem(t *testing.T) {
	c := check.New(t)
	table := tableTree()
	c.Equal(accessibility.NodeID(8), GridItem(table, 6, 1, 0))
	c.Equal(accessibility.NodeID(9), GridItem(table, 6, 1, 1))
	c.Equal(accessibility.NodeID(11), GridItem(table, 6, 2, 0))
	c.Equal(accessibility.NodeID(12), GridItem(table, 6, 2, 1))
	c.Equal(accessibility.NodeID(0), GridItem(table, 6, 0, 0), "a row outside the viewport is not published")
	c.Equal(accessibility.NodeID(0), GridItem(table, 6, 1, 5))
	c.Equal(accessibility.NodeID(0), GridItem(table, 6, -1, 0))
	c.Equal(accessibility.NodeID(0), GridItem(table, 6, 1, -1))
	c.Equal(accessibility.NodeID(0), GridItem(table, 999, 1, 0))
	c.Equal(accessibility.NodeID(0), GridItem(nil, 6, 1, 0))
	...
	c.Equal(accessibility.NodeID(0), GridItem(table, 6, 1, 0))
	c.Equal(accessibility.NodeID(8), GridItem(table, 6, 30, 0))
	...
	c.Equal(accessibility.NodeID(14), GridItem(nested, 6, 3, 1))
}

// TestTableColumnHeaders verifies how the header that describes a table's columns is found, which takes a search
// because the two are separate panels with nothing in the snapshot linking them.
func TestTableColumnHeaders(t *testing.T) {
	c := check.New(t)
	table := tableTree()
	c.Equal([]accessibility.NodeID{4, 5}, TableColumnHeaders(table, 6), "found by proximity")
	c.Nil(TableColumnHeaders(table, 999))
	c.Nil(TableColumnHeaders(nil, 6))
	c.Nil(TableColumnHeaders(table, 2), "a scroll area has no columns")
	...
	c.Equal([]accessibility.NodeID{4, 5}, TableColumnHeaders(controlled, 6))
	controlling := tableTree()
	controlling.Nodes[6].Controls = []accessibility.NodeID{3}
	c.Equal([]accessibility.NodeID{4, 5}, TableColumnHeaders(controlling, 6))
	...
	// modified, and TableColumnHeaders remembers its answer for the tree it was asked about.
	c.Equal([]accessibility.NodeID{4, 5}, TableColumnHeaders(buriedHeaderTree(false), 6))
	c.Nil(TableColumnHeaders(buriedHeaderTree(true), 6))
	...
	two := twoTableTree()
	c.Nil(TableColumnHeaders(two, 6))
	c.Nil(TableColumnHeaders(two, 13))
	named := twoTableTree()
	named.Nodes[6].Controls = []accessibility.NodeID{3}
	c.Equal([]accessibility.NodeID{4, 5}, TableColumnHeaders(named, 6))
	c.Nil(TableColumnHeaders(named, 13))
	...
	c.Nil(TableColumnHeaders(alone, 6))
	...
	c.Equal([]accessibility.NodeID{4, 5}, TableColumnHeaders(sibling, 6))
}

// TestTableColumnHeadersNested verifies that a table nested in a cell of another table claims no header at all
...
func TestTableColumnHeadersNested(t *testing.T) {
	c := check.New(t)
	nested := nestedTableTree()
	c.Nil(TableColumnHeaders(nested, 13), "the inner table's search stops at the table that contains it")
	c.Equal([]accessibility.NodeID{4, 5}, TableColumnHeaders(nested, 6), "the outer table is unaffected")
	c.Equal(accessibility.NodeID(0), ColumnHeaderItem(nested, 15), "so the inner table's cell has no header")
	c.Equal(accessibility.NodeID(5), ColumnHeaderItem(nested, 12), "while the cell holding it still has one")
}

// TestTableColumnHeadersMemo verifies that the answer remembered for one snapshot is never handed to another. The
// cost of finding a header is what makes remembering it worth doing — a client asks once per cell — but a window that
// publishes a new snapshot must be described by the new one from that moment on. See snapshotMemo.
func TestTableColumnHeadersMemo(t *testing.T) {
	c := check.New(t)
	first := tableTree()
	c.Equal([]accessibility.NodeID{4, 5}, TableColumnHeaders(first, 6))
	c.Equal([]accessibility.NodeID{4, 5}, TableColumnHeaders(first, 6), "asking twice answers the same")
	...
	c.Nil(TableColumnHeaders(headerless, 6), "the next snapshot has no header, and is not answered from the last")
	c.Equal([]accessibility.NodeID{4, 5}, TableColumnHeaders(first, 6), "and going back answers the first again")
}

// TestForgetSnapshotMemo verifies that the memo can be emptied, which is what keeps the last snapshot of a destroyed
// window — and every node in it — from staying reachable for the rest of the process. Window.Destroy calls it; the
// only thing a client can notice is that the next question is worked out from scratch.
func TestForgetSnapshotMemo(t *testing.T) {
	c := check.New(t)
	...
	c.Equal([]accessibility.NodeID{4, 5}, TableColumnHeaders(tree, 6))
	c.False(IsContentElement(tree, tree.Node(13)), "the label names the table, so it is not content of its own")
	position, size := PositionInSet(tree, tree.Node(7))
	c.Equal(2, position)
	c.Equal(5, size)

	forgetSnapshotMemo()
	c.Nil(snapshotMemo.tree, "the snapshot is no longer held")
	c.Nil(snapshotMemo.headers, "and neither is anything worked out from it")
	c.Nil(snapshotMemo.named)
	c.False(snapshotMemo.namedDone)
	c.Nil(snapshotMemo.positions)

	c.Equal([]accessibility.NodeID{4, 5}, TableColumnHeaders(tree, 6), "and the answer is worked out again")
	c.Equal(tree, snapshotMemo.tree)
	forgetSnapshotMemo()
}

// TestSnapshotMemoHoldsOneSnapshot verifies that every answer remembered is dropped the moment a different snapshot
// is asked about, which is what keeps one window's answers from ever being given for another's.
func TestSnapshotMemoHoldsOneSnapshot(t *testing.T) {
	c := check.New(t)
	forgetSnapshotMemo()
	...
	c.False(IsContentElement(named, named.Node(2)), "a label another node names is not in the content view")
	c.Equal(named, snapshotMemo.tree)
	c.True(snapshotMemo.namedDone, "and the whole snapshot's answer was worked out at once")

	c.True(IsContentElement(lone, lone.Node(2)), "while the next snapshot names nothing, and is not answered from"+
		" the last")
	c.Equal(lone, snapshotMemo.tree)
	c.Nil(snapshotMemo.named, "a snapshot in which nothing names anything allocates no set at all")
	c.False(IsContentElement(named, named.Node(2)), "and going back answers the first again")
	...
	c.False(IsContentElement(current, current.Node(2)), "the label the current snapshot names is not content")
	c.Equal(current, snapshotMemo.tree)
	c.True(IsContentElement(previous, previous.Node(2)), "the previous snapshot is answered from itself")
	c.Equal(current, snapshotMemo.tree, "without evicting the snapshot the memo is serving")
	c.True(snapshotMemo.named[2], "or dropping what had already been worked out from it")
	...
	other.Generation = 1
	c.False(IsContentElement(other, other.Node(12)), "another window's label names something of its own")
	c.Equal(other, snapshotMemo.tree, "and its snapshot takes the memo over")
}

// TestPositionInSetMemo verifies that the two halves of one node's position are worked out once and answered twice,
// which is what a client reading PositionInSet and SizeOfSet of the same element asks for.
func TestPositionInSetMemo(t *testing.T) {
	c := check.New(t)
	forgetSnapshotMemo()
	t.Cleanup(forgetSnapshotMemo)
	...
	c.Equal(2, position)
	c.Equal(2, size)
	c.Equal(setPosition{position: 2, size: 2}, snapshotMemo.positions[4])
	position, size = PositionInSet(tree, tree.Node(4))
	...
	position, size = PositionInSet(ignored, ignored.Node(2))
	c.Equal(0, position)
	c.Equal(0, size)
	c.Equal(tree, snapshotMemo.tree, "so the snapshot being answered from is the one that was asked about")
	...
	position, size = PositionInSet(other, other.Node(4))
	c.Equal(1, position)
	c.Equal(1, size)
	...
	// whose attributes changed — cannot evict the snapshot a client is walking. See memoSwitchTo.
	...
	position, size = PositionInSet(current, current.Node(4))
	c.Equal(2, position)
	c.Equal(2, size)
	c.Equal(current, snapshotMemo.tree)
	position, size = PositionInSet(previous, previous.Node(4))
	c.Equal(1, position, "the previous snapshot is answered from itself")
	c.Equal(1, size)
	c.Equal(current, snapshotMemo.tree, "without evicting the snapshot the memo is serving")
	c.Equal(setPosition{position: 2, size: 2}, snapshotMemo.positions[4], "or what it had remembered")
	c.Equal(1, len(snapshotMemo.positions), "and nothing of the older snapshot's is remembered")
}

// TestColumnHeaderItem verifies which header a cell says describes its column, including the fallback for a snapshot
// that never filled the headers' column indexes in.
func TestColumnHeaderItem(t *testing.T) {
	c := check.New(t)
	table := tableTree()
	c.Equal(accessibility.NodeID(4), ColumnHeaderItem(table, 8))
	c.Equal(accessibility.NodeID(5), ColumnHeaderItem(table, 9))
	c.Equal(accessibility.NodeID(4), ColumnHeaderItem(table, 11))
	c.Equal(accessibility.NodeID(5), ColumnHeaderItem(table, 12))
	c.Equal(accessibility.NodeID(0), ColumnHeaderItem(table, 999))
	c.Equal(accessibility.NodeID(0), ColumnHeaderItem(table, 4), "a header has no header")

	// Headers whose column indexes were never filled in are taken by position instead.
	unindexed := tableTree()
	unindexed.Nodes[5].ColumnIndex = 0
	c.Equal(accessibility.NodeID(4), ColumnHeaderItem(unindexed, 8))
	c.Equal(accessibility.NodeID(5), ColumnHeaderItem(unindexed, 9))
	...
	c.Equal(accessibility.NodeID(0), ColumnHeaderItem(short, 9))
}

// TestValueString verifies the text the Value pattern reports, and that a password reports nothing.
func TestValueString(t *testing.T) {
	c := check.New(t)
	tree := patternTree()
	c.Equal("Gandalf", ValueString(tree.Node(8)), "a text control's content is its value")
	c.Equal("fixed", ValueString(tree.Node(10)))
	c.Equal("Red", ValueString(tree.Node(14)))
	c.Equal("#ff0000", ValueString(tree.Node(16)))
	c.Equal("", ValueString(tree.Node(9)), "a password reports nothing")
	c.Equal("", ValueString(tree.Node(17)))
	c.Equal("", ValueString(nil))

	// An explicit value wins over the text, since a widget that fills in both means the value.
	tree.Nodes[8].Value = "Mithrandir"
	c.Equal("Mithrandir", ValueString(tree.Node(8)))
	...
	c.Equal("https://example.com", ValueString(link))
	link.Value = "something else"
	c.Equal("https://example.com", ValueString(link))
	c.Equal("", ValueString(&accessibility.Node{ID: 1, Role: role.Link, Name: "docs"}),
		"a link with no target has nothing to report, which is why it is given no Value pattern either")
}

// TestValueReadOnly verifies when the Value pattern says the value cannot be changed.
func TestValueReadOnly(t *testing.T) {
	c := check.New(t)
	tree := patternTree()
	c.False(IsValueReadOnly(tree.Node(8)), "a text field takes a new value")
	c.False(IsValueReadOnly(tree.Node(15)), "so does a combo box")
	c.True(IsValueReadOnly(tree.Node(10)), "not one the snapshot marks read-only")
	c.True(IsValueReadOnly(tree.Node(14)), "a popup button reports its choice but does not take one")
	c.True(IsValueReadOnly(tree.Node(16)), "nor does a color well")
	c.True(IsValueReadOnly(nil))

	tree.Nodes[8].Disabled = true
	c.False(IsValueReadOnly(tree.Node(8)),
	...
	c.True(IsValueReadOnly(tree.Node(8)), "a node that offers no SetValue action cannot take one")

	document := &accessibility.Node{
		ID: 1, Role: role.Document, Actions: accessibility.ActionSet(0).With(accessibility.SetValue),
	}
	c.True(IsValueReadOnly(document), "a document is there to be read")

	link := &accessibility.Node{
		ID: 1, Role: role.Link, URL: "https://example.com",
		Actions: accessibility.ActionSet(0).With(accessibility.SetValue),
	}
	c.True(IsValueReadOnly(link), "a link reports where it leads, and nothing retargets one through accessibility")
}

// TestRangeValueReadOnly verifies when the RangeValue pattern says the value cannot be changed.
func TestRangeValueReadOnly(t *testing.T) {
	c := check.New(t)
	tree := patternTree()
	c.False(IsRangeValueReadOnly(tree.Node(11)), "a slider takes a new value")
	c.False(IsRangeValueReadOnly(tree.Node(12)),
		"so does a disabled one, in principle: SetValue refuses it as not enabled rather than as read-only")
	c.True(IsRangeValueReadOnly(tree.Node(13)), "nor does a progress bar, ever")
	c.True(IsRangeValueReadOnly(nil))
	...
	c.True(IsRangeValueReadOnly(scrollBar), "offering no SetValue action is what makes a value read-only")
	scrollBar.Actions = scrollBar.Actions.With(accessibility.SetValue)
	c.False(IsRangeValueReadOnly(scrollBar), "a real scroll bar offers SetValue, so it is not read-only")
}
