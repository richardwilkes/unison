...
// Core takes over is Core's to destroy, while one built to pass into a UiaRaise* call is destroyed here.
...
func NewSafeArrayInt32(values []int32) SAFEARRAY {
	...
	for i := range values {
		if !hresultSucceeded(SafeArrayPutElement(array, int32(i), unsafe.Pointer(&values[i]))) {
			array.Destroy()
			return 0
		}
	}
...
func NewSafeArrayUnknown(values []unsafe.Pointer) SAFEARRAY {
	...
	for i, value := range values {
		if !hresultSucceeded(SafeArrayPutElement(array, int32(i), value)) {
			array.Destroy()
			return 0
		}
	}
...
func SafeArrayToInt32(array SAFEARRAY) []int32 {
	...
	if !hresultSucceeded(hr) || bound < 0 {
		return nil
	}
	values := make([]int32, bound+1)
	for i := range values {
		if !hresultSucceeded(SafeArrayGetElement(array, int32(i), unsafe.Pointer(&values[i]))) {
			return nil
		}
	}
	return values
