...
// eventSyntheticFlag is the high bit of an event's code, which the X server sets on an event that another client sent
// with SendEvent rather than one it generated itself. The code an event carries keeps the bit, and the event is decoded
// from the remaining seven bits, so [Event.ID] reports it intact.
const eventSyntheticFlag = 0x80
...
// Synthetic reports whether an event was sent by another client with SendEvent rather than generated by the X server.
// The distinction matters where the two say different things: a window manager is required to send a synthetic
// ConfigureNotify when it has moved a window without resizing it, and that one carries the window's position relative
// to the root rather than relative to its parent (ICCCM 4.2.3, "Configuring the Window").
func Synthetic(e Event) bool {
	return e != nil && e.ID()&eventSyntheticFlag != 0
}
