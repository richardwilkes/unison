...
func x11ScanCodeToKeySym(scanCode uint16, mods mod.Modifiers, level3 bool) uint32 {
	...
	syms := (scanCode - uint16(x11Conn.MinKeyCode)) * per
...
func isKeypadKey(keySym uint32) bool {
	return keySym >= xkKPSpace && keySym <= xkKPEqual
}

func isPrivateKeypadKey(keySym uint32) bool {
...
func x11KeySymToUnicode(keySym uint32) rune {
