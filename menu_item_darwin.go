...
func (mi *macMenuItem) KeyBinding() KeyBinding {
	...
	return KeyBinding{KeyCode: macMenuEquivalentToKeyCodeMap[keyStr], Modifiers: macModifiersFromEventModifierFlags(mods)}
}
...
var (
	macKeyCodeToMenuEquivalentMap = map[KeyCode]string{
		KeySpace:        " ",
		KeyApostrophe:   "'",
		KeyComma:        ",",
		KeyMinus:        "-",
	...
	macMenuEquivalentToKeyCodeMap map[string]KeyCode
)

func init() {
	macMenuEquivalentToKeyCodeMap = make(map[string]KeyCode, len(macKeyCodeToMenuEquivalentMap))
	for k, v := range macKeyCodeToMenuEquivalentMap {
		macMenuEquivalentToKeyCodeMap[v] = k
	}
}
