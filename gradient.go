...
func (g *Gradient) Paint(_ *Canvas, rect geom32.Rect, style PaintStyle) *Paint {
	paint := NewPaint()
	...
	if g.StartRadius > 0 && g.EndRadius > 0 {
		shader = New2PtConicalGradientShader(start, end, g.StartRadius, g.EndRadius, colors, colorPos, TileModeClamp,
			geom32.NewIdentityMatrix2D())
	} else {
		shader = NewLinearGradientShader(start, end, colors, colorPos, TileModeClamp, geom32.NewIdentityMatrix2D())
	}
